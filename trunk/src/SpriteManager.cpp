...
Mob2DNode SpriteManager::AddNode(string handle)
{
	if(sprites.find(handle) != sprites.end())
	{
		Mob2DNode node(new mob2d_node(sprites[handle]));
		nodes.insert(std::pair<string, Mob2DNode>(handle, node));
		return node;
	}
	else
	{
		Mob2DNode node(new mob2d_node(sprites["ERROR"]));
		Mob2DLog::Instance()->PushString("Mob2DNode set to error and left unmapped.");
		return node;
	}
}
void SpriteManager::MapNode(Mob2DNode node, string key)
{
    // Ain't I clever?
    if(sprites.find(key) == sprites.end())
        Mob2DLog::Instance()->PushString("ERROR! Could not map node to a nonexistant key.");
    else
        nodes.insert(std::pair<string, Mob2DNode>(key, node));
}
